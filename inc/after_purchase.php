    ...
    <div class="container">
        <div class="grid">
            <div id="orderInfo" class="col-8_sm-12">
                <h4><?php echo __('Affichage des commandes', 'woocommerce'); ?></h4>

                <div class="grid articles">
                    <div class="col-6 label"><?php echo __('Nom du produit', 'woocommerce'); ?></div>
                    <div class="col-2 text-right label"><?php echo __('Devis', 'woocommerce'); ?></div>
                    <div class="col-2 text-right label"><?php echo __('Quantité', 'woocommerce'); ?></div>
                ...
                <div class="grid summary">
                    ...
                    <div class="col-8 text-right label"><?php echo __('Sous-total', 'woocommerce'); ?></div>
                    <div class="col-4 text-right"><?php echo wp_kses_post($order_subtotal); ?></div>
                    <div class="col-8 text-right label">
                        <?php echo __('Expédition', 'woocommerce') . '<span class="wo-tax"> (hors TVA)</span>'; ?></div>
                    ...
                    <?php
                            foreach ( $order->get_fees() as $fee ) {
                                ...
                                echo '<div class="col-8 text-right label">' . __(esc_html( $fee_name ), 'nutrisslim-suite') . '<span class="wo-tax"> (hors TVA)</span>' .'</div>';
                    ...
                    <div class="col-8 text-right"><?php echo __('(TVA incluse)', 'woocommerce'); ?>
                    </div>
            ...
            <div id="customerInfo" class="col-4_sm-12">
                <div class="grid shipping">
                    ...
                    <div style="padding-bottom:0" class="col-12">
                        <h4><?php echo __('Adresse de livraison et de facturation', 'nutrisslim-suite'); ?></h4>
                    </div>
                    <div class="col-12"><?php echo wp_kses_post($billing_address); ?></div>
                    <?php else : ?>
                    <div style="padding-bottom:0" class="col-12">
                        <h4><?php echo __('Adresse de livraison', 'woocommerce'); ?></h4>
                    </div>
                    <div class="col-12"><?php echo wp_kses_post($shipping_address); ?></div>
                    <div style="padding-bottom:0" class="col-12">
                        <h4><?php echo __('Adresse de facturation', 'woocommerce'); ?></h4>
                    </div>
                    <div class="col-12"><?php echo wp_kses_post($billing_address); ?></div>
                    <?php endif; ?>
                </div>
            ...
            <div class="col-12 conclusion">
                <p class="text-center">
                    <?php echo __('Merci pour votre achat. Nous vous souhaitons une excellente journée!', 'nutrisslim-suite'); ?>
                </p>
                <p class="text-center"><?php echo __('Votre Nature\'s Finest', 'nutrisslim-suite'); ?></p>
            </div>
        </div>
    </div>
</section>
...
function thankyouContent() {
	...
	if ( is_checkout() && is_wc_endpoint_url('order-received') ) {
		...
		if ( (in_array($method, ['cod', 'stripe'])) && $enabled && ($is_afterpurchase !== 'yes') ) {
			...
			echo '<h3>' . $first_name . ', ' . __('La commande a été complétée', 'nutrisslim-suite') . '</h3>';
            echo '<p>' . __('Merci d\'avoir choisi nos produits.', 'nutrisslim-suite') . '</p>';
            echo '<p>' . __('Numéro de commande', 'woocommerce') . ': <strong>' . $order_id . '</strong></p>';
            echo '<p>' . sprintf(__('La confirmation de commande sera envoyée à l\'adresse e-mail %1$s.', 'text-domain'), '<strong>' . esc_html($order_email) . '</strong>') . '</p>';
			echo '<p><a id="pregled-narucila" data-oid="' . $order_id . '" class="btn wired-btn elementor-button" href="#">' . __('Aperçu de la commande', 'nutrisslim-suite') . ':</a></p>';
            echo '<div class="timer-after-purchase">';
            echo '<h3>' . __('Offre spéciale', 'nutrisslim-suite') . '</h3>';
            echo '<p>' . __('Réduction supplémentaire sur une sélection de produits pour une durée limitée !!!
            ', 'nutrisslim-suite') . '</p>';
            ...
            if ($has_book_file) {
                ...
                foreach ($order->get_items() as $item_id => $item) {
                    ...
                    if ($book_file) {
                        ...
                        echo '<p class="text-center"><a href="' . esc_url($book_file['url']) . '" target="_blank">' . __('Télécharger le livre', 'nutrisslim-suite') . '</a></p>';
                        echo '</div>'; // col
                        echo '</div>';
                    }
                }
			...
			echo '<div class="container">';
			echo '<h3>' . sprintf(esc_html__('%1$s, la commande a été complétée', 'text-domain'), esc_html($first_name)) . '</h3>';
            echo '<p><strong>' . __('Merci d\'avoir choisi nos produits.', 'nutrisslim-suite') . '</strong></p>';

            echo '<p>' . __('Chez Nature\'s Finest, nous nous engageons à offrir les meilleures solutions naturelles pour un mode de vie sain – et grâce à vous, nous pouvons continuer à concrétiser cette vision !', 'nutrisslim-suite') . '</p>';

            echo '<p>' . __('Nous vous informerons dès que votre colis sera prêt.', 'nutrisslim-suite') . '<br /><br /></p>';
            
            echo '<p>' . __('Numéro de commande', 'woocommerce') . ': <strong>' . $order_id . '</strong></p>';
            
            echo '<p>' . sprintf(__('La confirmation de commande sera envoyée à l\'adresse e-mail fournie.
            <strong>%1$s</strong>.', 'nutrisslim-suite'), esc_html($order_email)) . '</p>';
            if ($order && $order->get_status() === 'pending') {
                echo '<p><a id="show-offer" data-oid="' . $order_id . '" class="btn wired-btn elementor-button" href="#">' . __('Afficher une offre supplémentaire :', 'nutrisslim-suite') . '</a></p>';
            }
...
<?php
            ...
            $ato = sprintf( __( 'Ajouter à la commande', 'nutrisslim-suite') );
            $rfo = sprintf( __( 'Supprimer de la commande', 'nutrisslim-suite') );            
            ...
            foreach ($items as $item) {
                ...
                data-offer-type="initial">' . $ato . '</button></div>';                echo '</div>';
                echo '</div>';	
            }
            ...
            if (in_array($method, ['cod'])) {
                ...
                _e('Finaliser la commande', 'nutrisslim-suite');
                echo '</a>';    
            } else {
                ...
                _e('Terminer la commande', 'nutrisslim-suite');
                echo '</a>';    
            }
            echo '<p class="text-center"><a id="straighttofinal" data-oid="' . $order_id . '" href="#">' . __( 'Je ne demande pas cette offre', 'nutrisslim-suite') . '</a></p>';
            echo '</div>';
            ...
            echo '<h3>' . sprintf(__('<span>Dernière chance</span><br /> Profitez de la réduction de %1$s%%!', 'nutrisslim-suite'), esc_html($discount_percentage)) . '</h3>';
            ...
            echo '<p class="text-center"><a class="gotopregled final" data-oid="' . $order_id . '" href="#">' . __( 'Je ne souhaite pas cette offre', 'nutrisslim-suite') . '</a></p>';            
            echo '</div>';		
            echo '</div>';
            echo '</div>';		
			...
			echo '<div class="container">';
			echo '<h3>' . sprintf(esc_html__('%1$s, La commande a été traitée', 'text-domain'), esc_html($first_name)) . '</h3>';
            echo '<p><strong>' . __('Merci d\'avoir choisi nos produits.', 'nutrisslim-suite') . '</strong></p>';        
            echo '<p>' . __('Chez Nature\'s Finest, nous nous engageons depuis 10 ans à offrir les meilleures solutions naturelles pour une vie saine – et grâce à vous, nous pouvons continuer à concrétiser cette vision !', 'nutrisslim-suite') . '</p>';

            echo '<p>' . __('Nous vous informerons immédiatement dès que votre colis sera prêt.', 'nutrisslim-suite') . '<br /><br /></p>';
            
            echo '<p>' . __('Numéro de commande', 'woocommerce') . ': <strong>' . $order_id . '</strong></p>';
            
            echo '<p>' . sprintf(__('La confirmation de commande sera envoyée à l\'adresse e-mail fournie sur le site. <strong>%1$s</strong>.', 'nutrisslim-suite'), esc_html($order_email)) . '</p>';            if ($order && $order->get_status() === 'pending') {
                echo '<p><a id="show-offer" data-oid="' . $order_id . '" class="btn wired-btn elementor-button" href="#">' . __('Afficher une offre supplémentaire:', 'nutrisslim-suite') . '</a></p>';
            }
            ...
            if ($has_book_file) {
                ...
                foreach ($order->get_items() as $item_id => $item) {
                    ...
                    if ($book_file && !in_array($book_file['ID'], $displayed_files)) {
                        ...
                        echo '<h3>' . esc_html($book_file['title']) . '</h3>';
                        echo '<p>' . esc_html($book_file['description']) . '</p>';
                        echo '<p class="text-center"><a href="' . esc_url($book_file['url']) . '" target="_blank">' . __('Téléchargez le livre', 'nutrisslim-suite') . '</a></p>';
                        echo '</div>'; // col
                        echo '</div>';
                    }
                }
...
<h4><?php echo __('Consultez vos commandes', 'woocommerce'); ?></h4>
...
<div class="grid summary">
    ...
    <div class="col-4 text-right"><?php echo wp_kses_post($order_subtotal); ?></div>
    <div class="col-8 text-right label">
        <?php echo __('Expédition', 'woocommerce') . '<span class="wo-tax"> (bez PDV)</span>'; ?></div>
    <div class="col-4 text-right"><?php echo wp_kses_post($order_shipping); ?>
    </div>
    ...
    <?php
                            foreach ( $order->get_fees() as $fee ) {
                                ...
                                // Output the fee name and total including tax
                                echo '<div class="col-8 text-right label">' . __(esc_html( $fee_name ), 'nutrisslim-suite') . '<span class="wo-tax"> (bez PDV)</span>' .'</div>';
                                echo '<div class="col-4 text-right">' . $fee_total_incl_tax . '</div>';
                            }
                        ?>
    ...
    <div class="col-8 text-right label znesek"><?php echo __('Total', 'woocommerce'); ?></div>
    <div class="col-4 text-right"><?php echo wp_kses_post($order_total); ?></div>
    <div class="col-8 text-right"><?php echo __('(z VAT)', 'woocommerce'); ?>
    </div>
    <div class="col-4 text-right"><?php echo wp_kses_post($order_tax); ?></div>
</div>
