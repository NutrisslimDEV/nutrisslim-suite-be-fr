...
<?php
    ...
    if ($product_type == 'nutrisslim') {
        // $singleLabel = ' paket';
        $singleLabel = __('colis', 'nutrisslim-suite');
        $dvaLabel = 'colis';
        // $triLabel = ' paketi';
        $triLabel = __('colis', 'nutrisslim-suite');
    } else {
        // $singleLabel = ' kom';
        $singleLabel = __('pièce', 'nutrisslim-suite');
        $dvaLabel = 'kom'; 
        $triLabel = 'kom';                    
        // $triLabel = ' kom';                    
        $triLabel = __('pièces', 'nutrisslim-suite');
    }
    ...
    */
    if ($sale_price) {
        $subscribe_sale_price = getDiscountedPrice($sale_price, 5);
        $discount_one_subscription = round((($regular_price - $subscribe_sale_price) / $regular_price) * 100);
    }
    if ($price_for_two) {
        $subscribe_price_for_two = getDiscountedPrice($price_for_two, 5);
        $discount_two_subscription = round((($regular_price - $subscribe_price_for_two) / ($regular_price)) * 100);
    }
    if ($price_for_three) {
        $subscribe_price_for_three = getDiscountedPrice($price_for_three, 5);
...
<div class="customAddToCart">
    <div class="topInfo">
        ...
        <span class="subscriptiolLabel" style="display:none;">
            <?php printf(__('pendant  <span class="numdays">%s</span> jours', 'nutrisslim-suite'), $subscription_periods[0]); ?></span>
    </div>
    <p><?php echo __('Choisissez la quantité de produits:', 'nutrisslim-suite'); ?></p>
        ...
        <form>
            <div class="qSelect grid-3">
                ...
                <label class="col top">
                    ...
                    <div class="paket">
                        ...
                        <div class="choice">
                            <?php echo __('Le meilleur choix', 'nutrisslim-suite'); ?>
                        </div>
                    </div>
                </label>
                <?php        
            ...
            <div class="tabsHolder">
                <div class="tabsItems grid-2">
                    <div class="col tab subscr">
                        <?php echo __('Abonnez-vous et économisez', 'nutrisslim-suite'); ?><br />
                        <span class="warning">-<?php echo $discount_one_subscription; ?>%
                            <?php echo __('REMISE', 'nutrisslim-suite'); ?><br /></span>
                    </div>
                    <div class="col tab active onetime"><?php echo __('Achat unique', 'nutrisslim-suite'); ?>
                    </div>
                </div>
                <div class="contWrap">
                    <div class="onetimeContent">
                        <p>
                            <strong>
                                <?php 
                                    printf( 
                                        __( 'Passez à l’abonnement et économisez %s', 'nutrisslim-suite' ), 
                            ...
                            <?php 
                                printf( 
                                    __( 'Vous pouvez avoir la même chose pour seulement %s. Une façon pratique d’obtenir les meilleurs prix. L’abonnement peut être modifié ou annulé à tout moment! ', 'nutrisslim-suite' ), 
                    ...
                    <div style="display:none;" class="subscrContent">
                        <p><?php echo __('Choisissez la période d\'abonnement :', 'nutrisslim-suite'); ?></p>
                        <select name="subscription" class="subscription">
                            <?php
                                    foreach ($subscription_periods as $subscription_period) {
                                        echo '<option value="' . $subscription_period . '">' . __('Envoyez tous les', 'nutrisslim-suite') . ' ' . $subscription_period . ' ' . __('jours', 'nutrisslim-suite') . '</option>';
                                    }
                                ?>
                        </select>
                        <div class="grid info">
                            <div class="col-6 cancel">
                                <?php echo __('L’abonnement peut être annulé à tout moment.', 'nutrisslim-suite'); ?>
                            </div>
                            <div class="col-6 lowest">
                                <?php echo __('Prix le plus bas vérifié', 'nutrisslim-suite'); ?>
                            </div>
                            <div class="col-12 learn"><a class="moreabout"
                                    href="#"><?php echo __('En savoir plus sur l’abonnement', 'nutrisslim-suite') ?></a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <?php
                    if ( !$product->is_in_stock() ) {
                        ...
                        echo '<p style="text-align:center"><span class="out-of-stock">' . __( 'Rupture de stock', 'woocommerce' ) . '</span></p>';
                    } else {
                ?>
            <button class="elementor-button add-to-cart-icon" data-product-id="<?php echo $product->get_id(); ?>"
                data-quantity="1" type="submit"><?php echo __('Ajouter au panier', 'woocommerce'); ?> | <span
                    class="buttonPrice"><?php echo wc_price($sale_price); ?></span> <span class="buttonTime"
                    style="display:none;"><?php printf(__('chaque  <span class="numdays">%s</span> jour', 'nutrisslim-suite'), $subscription_periods[0]) ?></span></button>
            <?php
            ...
            <p <?php echo $vis; ?>class="prihranek">
                <?php printf(__('Vous économisez <span class="prihrana">%s</span>', 'nutrisslim-suite'), wc_price($regular_price - $sale_price)); ?>
            </p>
            ...
            <?php
                    if ( $product->is_in_stock() ) {
                        // If the product is out of stock, display "Out of Stock" text
                        echo '<p class="onstock">' . __( 'En stock, prêt à être expédié', 'nutrisslim-suite' ) . '</p>';
                    }                
                ?>
    ...
    <div class="featureWrapper">
        <div class="feature return"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/cart_vracilo_ico.svg" />
            <?php printf(__('Garantie de remboursement de %s jours', 'nutrisslim-suite'), 60); ?></div>
        <div class="feature free"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/cart_free_delivery_ico.svg" />
            <span><?php printf(__('Livraison gratuite pour les commandes dès %s', 'nutrisslim-suite'), wc_price(get_free_shipping_threshold())); ?></span>
        </div>
        <div class="feature delivery"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/cart_dostava_ico.svg" />
            <?php printf(__('Livraison sous 3 à 4 jours ouvrables', 'nutrisslim-suite'), '1-2'); ?></div>
    </div>
    ...
    <p class="phone">
        <?php printf(__('Les commandes et demandes de renseignements peuvent être adressées au numéro <strong>%s</strong>', 'nutrisslim-suite'), get_option('woocommerce_support_phone')); ?>
    </p>
</div>
<?php
    }
