...
class Nutrisslim_Product_Tabs_Widget extends Widget_Base {
    ...
    protected function render() {
        ...
        if ( ! empty( $uporaba_content ) ) {
            $tabs['uporaba'] = array(
                'title'    => __( 'Utilisation', 'nutrisslim-suite' ),
        ...
        if ( ! empty( $sestavine_content ) ) {
            $tabs['sestavine'] = array(
                'title'    => __( 'Ingrédients', 'nutrisslim-suite' ),
        ...
        if ( ! empty( $hranilne_vrednosti_content ) ) {
            $tabs['hranilne_vrednosti'] = array(
                'title'    => __( 'Valeurs nutritionnelles', 'nutrisslim-suite' ),
        ...
        $tabs['reviews'] = array(
            'title'    => __( 'Avis', 'nutrisslim-suite' ),
...
function ns_render_merged_reviews_panel( int $product_id ) {
    ...
    echo '    <h2 class="woocommerce-Reviews-title">' . esc_html__( 'Avis des clients', 'woocommerce' ) . '</h2>';
...
function nutrisslim_uporaba_tab_content_callback() {
    ...
    if ($warnings) {
        echo '<p><strong>' . __( 'Avertissement:', 'nutrisslim-suite' ) . '</strong></p>';
        echo $warnings;
    }
}
